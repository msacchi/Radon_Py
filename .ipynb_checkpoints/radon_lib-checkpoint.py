...
def radon_cg(d,m0,Nt,dt,Nh,h,Np,p,href):
...
# M D Sacchi, 2015,  Email: msacchi@ualberta.ca
    ...
    s = d-radon_forward(m,Nt,dt,Nh,h,Np,p,href)
    pp = radon_adjoint(s,Nt,dt,Nh,h,Np,p,href)
    r = pp
    ...
    for k in range(0,15):
        alpha = np.sum(np.sum(r*r))/np.sum(np.sum(q*q))
            m +=  alpha*pp
            s -=  alpha*q
            r = radon_adjoint(s,Nt,dt,Nh,h,Np,p,href)
